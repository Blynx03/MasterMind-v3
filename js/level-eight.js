...
const winnerSound = new Audio("sound/winsound2.wav");

// Countdown

function levelTimer(secs) {
  const timerSound = new Audio("sound/timer3.wav");
  const gameOverSound = new Audio("sound/gameover3.wav");
  ...
  setTimeout(function () {
    ...
    let stopInterval = setInterval(function () {
      ...
      if (secs <= 10) {
        timerSound.volume = 1;
        divText.style.backgroundImage =
          "linear-gradient(35deg, black 5%, #5d0404 15%, #fc6363 45%, #fc6363 55%, #5d0404 85%, black 95%";
        divText.style["-webkit-animation"] = "animate-blink 10000ms linear";

        const docContainer = document.querySelector(".curtain");
        docContainer.style["-webkit-animation"] =
          "animate-last-10sec 10000ms linear";
      }
  ...
  for (let i = 0; i <= 3; i++) {
    ...
    switch (randomNumbers[i]) {
      ...
      case 5:
        ...
        break;
      case 6:
        generatedColors[i] = "white";
        break;
      case 7:
        generatedColors[i] = "pink";
        break;
      case 8:
        generatedColors[i] = "black";
        break;
    }
  }
}
...
function assignColors(color, peg) {
  switch (color) {
    ...
    case "orange":
      ...
      break;
    case "white":
      peg.style.background = "rgba(255, 255, 255, 0.85)";
      break;
    case "pink":
      peg.style.background = "rgba(250, 3, 233, 0.85)";
      break;
    case "black":
      peg.style.background = "rgba(0, 0, 0, 0.85)";
      break;
  }
}
...
function checkAndShowFeedback() {
  ...
  if (feedback.includes("white") || feedback.includes("transparent")) {
    ...
    count = 0;
    if (attempt > 8) {
      // change 1 to 8 when done debugging
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          newContent1.textContent = "Congratulations!";

          newContent2.className = "level level-description";
          newContent2.textContent =
            "You are the chosen One! You are the ULTIMATE CODE BREAKER. Congratulations!";
          ...
          setTimeout(function () {
            ...
            newButton.setAttribute("onclick", "loadStartOver()");
          }, 2500);
        }, 200);
      }, 200);
    }, 3100);
