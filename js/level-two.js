...
setTimeout(function() {
    for (let i = 1; i <= 9; i++) {
        ...
        // colorsContainer2.style.visibility = "visible";
    }    
    const bodyContainer = document.querySelector(".body-container");
...
// let lineGrid = document.querySelector(".line-container2");
// lineGrid.style.gridArea = "line1";
...
function checkAndShowFeedback() {
    ...
    // console.log("feedback = " + feedback[0] + ", " + feedback[1] + ", " + feedback[2] + ", " + feedback[3] );
    for (let k = 0; k < feedback.length; k++) {
        let feedPeg = document.querySelector(".feed" + (k+1) + ".feedresult" + attempt);
        feedPeg.style.background = feedback[k]; 
    ...
    if (feedback.includes("white") || feedback.includes("transparent")) {
        ...
        feedback = [];
        count = 0;
        if (attempt > 8) {  // change 1 to 10 when done debugging
        ...
        setTimeout(function() {
            ...
            setTimeout(function() {
                ...
                setTimeout(function() {
                    ...
                    newContent1.textContent = "Level 3 Unlocked!";
                    newContent2.className = "level level-description";
                    newContent2.textContent = "Impressive. You will need better concentration on the next level as the feedbacks will no longer be in order.  Number of color choices and attempts will be put back to the original six and ten respectively.";
...
function showTryAgainBtn() {
    ...
    // tryBtn.style.animation = "animate-btn-appear 1500ms linear forwards";
